program cago
  ...
  implicit none
  !----------------------------------------
  integer,parameter::traj_unit=98
  integer,parameter::sample_unit=16
  integer,parameter::nlocal=4
  integer,parameter::unfold_step=30000
  !----------------------------------------
  real,parameter::pi=3.14159265
  real,parameter::vdwr=4.0
  real,parameter::vdwr12=vdwr**12
  real,parameter::deg2rad=pi/180.
  real,parameter::rad2deg=180/pi
  real,parameter::dt=0.05
  real,parameter::tcoup=0.3
  real,parameter::unfold_temp=3.0
  real,parameter::friction=0.1
  !----------------------------------------
  ...
  real,parameter::k_bond=100     
  real,parameter::k_anglebend=20 
  real ::k_dih1  
  real ::k_dih3
  real :: expo
  ...
  integer::sstep
  integer::reclen
  integer::num_consec_rmsd
  integer::ios
  integer,dimension(3000)::iseq
  integer,dimension(:),allocatable::seed
  real::sys_temp
  real::fold_temp
  real::rep_rmsd !rmsd that gets checked against future rmsds for constiency (never printed)
  real::q
  real::rmsd
  real::curr_temp
  real::t1
  ...
  real,dimension(:),allocatable::target_anglebend
  real,dimension(:),allocatable::target_dihedral
...
! to angd
  integer,dimension(:),pointer::sumr !stores overall rotation at the end of the run
  !real,dimension(:),allocatable::prot
  real,dimension(:,:),allocatable::native
  real,dimension(:,:),allocatable::model
  integer,dimension(:,:),allocatable::dsbonds
  real,dimension(:,:),allocatable::net_force
  real,dimension(:,:),allocatable::anglebend_f
  real,dimension(:,:),allocatable::dihedral_f
  real,dimension(:,:),allocatable::contact_f
  real,dimension(:,:),allocatable::cmap
  real,dimension(:,:),allocatable::bond_f
  real,dimension(:,:),allocatable::velocity
  real,dimension(:,:),allocatable::random_f
  real,dimension(:,:),allocatable::ljrep_f
  real,dimension(:,:),allocatable::acceleration
  real,dimension(:,:),allocatable::dsbond_f
  !------------------------------------------------------------------------------------
  integer::countl
  integer::ires
  integer::seed_1
  ...
  if(jarg < 11) then
    write(*,*)
    write(*,*)'usage: xcago {native pdb} {chain id} {trajectory} {energy} {temp}'// &
     " {nstep} {sample step} {dihedral_k} {dihedral_expo} {Start residue} {End_residue}"
    write(*,*)
    stop
  endif
  call flush(6)
  ...
  if(ierr /= 0) then
    write(*,*) "error xcago :: dihedral_k is not a number! ",sstep
  endif
  k_dih3 = k_dih1*.5
  call getarg(9,aline)
  read(aline,*,iostat=ierr)expo
  if(ierr /= 0) then
    write(*,*) "error xcago :: dihedral_expo is not a number! ",sstep
  endif
  ...
  !------------------------------------------------------------------------------------
  open(sample_unit,file=sample,iostat=ierr)
  if(ierr /= 0) stop 'error xcago :: cannot open sample file'
  !------------------------------------------------------------------------------------
  if(traj_unit /= 99) then
    open(traj_unit,file=traj,status='replace',iostat=ierr)
    if(ierr /= 0) stop 'Cannot open trajectory file!'
  else
    reclen=12*nres+8
    open(traj_unit,file=traj,access='direct',form='unformatted',recl=reclen,iostat=ierr,status='replace')
    if(ierr /= 0) stop 'Cannot open trajectory file!'
  endif
  ...
  write(unit=6,fmt="(8I12)") seed
  write(*,*) "no seg fault yet"
  call flush(6)
  !------------------------------------------------------------------------------------
  write(sample_unit,*)"---------------------------------------------------------------------"
  ...
  !nres=go_e-go_s+1
  !endif
  allocate(       ljrep_f(3,nres))
  allocate(         model(3,nres))
  allocate(     net_force(3,nres))
  allocate(        bond_f(3,nres))
  allocate(      random_f(3,nres))
  allocate(      velocity(3,nres))
  allocate(   anglebend_f(3,nres))
  allocate(    dihedral_f(3,nres))
  allocate(     contact_f(3,nres))
  allocate(  acceleration(3,nres))
  allocate( target_dihedral(nres))
  allocate(target_anglebend(nres))
  allocate(      dsbond_f(3,nres))
  ...
  call write_traj(traj_unit,model,nres,0,iseq,sys_temp)
...
!        !call lj_repulsion_nrg(model,ljrep_e,nres)
        ...
        call write_traj(traj_unit,model,nres,istep,iseq,sys_temp)
        ...
        write(sample_unit,'(i10,7f10.2,2f6.2,2f6.6,a,2f6.6,a,2i5)')&
  ...
  if(allocated(net_force))           deallocate(net_force)
  if(allocated(target_dihedral))     deallocate(target_dihedral)
  if(allocated(target_anglebend))    deallocate(target_anglebend)
  if(allocated(bond_f))              deallocate(bond_f)
  if(allocated(random_f))            deallocate(random_f)
  if(allocated(anglebend_f))         deallocate(anglebend_f)
  if(allocated(dihedral_f))          deallocate(dihedral_f)
  if(allocated(acceleration))        deallocate(acceleration)
  if(allocated(ljrep_f))             deallocate(ljrep_f)
  if(allocated(dsbonds))             deallocate(dsbonds)
  if(allocated(dsbond_f))            deallocate(dsbond_f)
  do countl=1,ndsbonds
     if(sumr(countl)>0) then
       write(sample_unit,*) 'The overall rotation for disulfide bond '&
       ,countl,' was ',abs(rotd),'degrees to the right'
     endif
     if(sumr(countl)<0) then
       write(sample_unit,*) 'The overall rotation for disulfide bond ' &
       ,countl,' was ',abs(rotd),'degrees to the left'
     endif
     if(sumr(countl)==0) then
       write(sample_unit,*),'No rotation observed for disulfide bond ',countl
     endif
  enddo
  call flush(sample_unit)
  close(traj_unit)
...
!----------------------------------------------------------------------------------------------------------------------------
  subroutine contact_force(xyz,cmap,contactf,nres)
   implicit none
   integer,intent(in)::nres
   real,dimension(:,:),intent(inout)::xyz,contactf
   real,dimension(:,:),intent(in)::cmap
   real::x,dij,dijsq,dij3,dij6,dij10,dij12,sig12
   real,dimension(3)::rij
   integer::ires,jres
...
!----------------------------------------------------------------------------------------------------------------------------
  subroutine random_force(model,ran_f,t,zi,dt,nres)
   ...
   real,dimension(:,:),intent(out)::ran_f
   !---------------------------------------
   integer::jres
   integer::i
   real::x
...
!----------------------------------------------------------------------------------------------------------------------------
  subroutine dihedral_force(xyz,dih_f,nres,targt)
   ...
   integer,intent(in)::nres
   real,dimension(:,:),intent(in)::xyz
   real,dimension(:,:),intent(out)::dih_f
   real,dimension(:),intent(in)::targt
   !---------------------------------------
   integer::ires
   real::x
   ...
   do ires=2,nres-2
     ...
     a =angle*deg2rad
     ao=targt(ires)*deg2rad
     !x = k_dih1*(sin(a-ao)) + k_dih3*3*(sin(3*(a-ao))) 
     x = (expo*(k_dih1*(1+cos(a-ao))+k_dih3*(1+cos(3*(a-ao))))**(expo-1))*(k_dih1*sin(a-ao) + k_dih3*3*sin(3*(a-ao))) &
     /((k_dih1+k_dih3)*2)**(expo-1)
     !if(ires/=go_s .and. ires/=go_e) then
...
!----------------------------------------------------------------------------------------------------------------------------
  subroutine anglebend_force(xyz,bend_f,nres,targt)
   ...
   real,dimension(:),intent(in)::targt
   real,dimension(:,:),intent(in)::xyz
   real,dimension(:,:),intent(out)::bend_f
   !---------------------------------------
   integer::ires
   real::angle
   real::x
...
!----------------------------------------------------------------------------------------------------------------------------
  subroutine update_dihedral(angs,xyz,nres)
   ...
   integer,intent(in)::nres
   real,dimension(:,:),intent(in)::xyz
   real,dimension(:),intent(out)::angs
   !---------------------------------------
   integer::ires
   real::sb
   real::sb2
...
!----------------------------------------------------------------------------------------------------------------------------
  subroutine get_cmap(protein,nres,cmap) 
   ...
   real,dimension(:,:),allocatable,intent(inout)::cmap
   !------------------------
   integer::c
   integer::iatom
   integer::jatom
...
!----------------------------------------------------------------------------------------------------------------------------
   ...
   implicit none
   !---------------------------------------
   integer,dimension(:,:),intent(in)::dsbonds
   real,dimension(:,:),intent(in)::xyz
...
!----------------------------------------------------------------------------------------------------------------------------
  subroutine sdhdir(angpr,angdr,rotdr,sumrr)
   ...
   integer,dimension(:),pointer::sumrr
   !-----------------------------------------
   ...
   do i=1,ndsbonds
     ...
     if (rotdr(i) >0) then
        !write(*,*)'before'
        sumrr(i)=sumrr(i)+1 !anti-clokwise flag wrt corkscrew rule
        !write(*,*)'Rotd ',rotdr(i)
     else if(rotdr(i)<0) then
         !write(*,*)'before'
        sumrr(i)=sumrr(i)-1 !clockwise flag wrt corkscrew rule
        !write(*,*)'Rotd',rotdr(:)
     endif
   enddo
   angpr=angdr 
...
!----------------------------------------------------------------------------------------------------------------------------  
  subroutine bond_force(xyz,nres,bondf)
   ...
   integer,intent(in)::nres
   real,dimension(:,:),intent(inout)::xyz
   real,dimension(:,:),intent(inout)::bondf
   !------------------------------------------
   integer::ires
   integer::jres
   real::dij
  ...
  subroutine dsbond_force(xyz,native_s,dsbonds,ndsbonds,nres,dsbondf)
   ...
   integer,intent(in),dimension(:,:)::dsbonds
   real,dimension(:,:)::native_s
   real,dimension(:,:),intent(inout)::xyz
   real,dimension(:,:),intent(inout)::dsbondf
   !------------------------------------------
   integer::ires
   integer::jres
   logical::isInDSBond
...
!----------------------------------------------------------------------------------------------------------------------------
  subroutine set_target_dihedral(dih,nres)
   ...
   !------------------------------------------
   integer,intent(in)::nres
   real,dimension(:),intent(inout)::dih
   !------------------------------------------
   integer::ires
   !------------------------------------------
   do ires=1,nres
